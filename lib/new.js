...
copyAssets = function(proj, tmpl, cb) {
  ...
  log('Creating Assets...');
  copy = function(dir) {
    return checkexists("" + __dirname + "/../templates/" + tmpl + "/" + dir, function(exists) {
      try {
        fs.mkdirSync("./" + proj + "/" + dir);
        return wrench.copyDirSyncRecursive("" + __dirname + "/../templates/" + tmpl + "/" + dir, "./" + proj + "/" + dir);
      } catch (err) {
        if (process.env.NODE_ENV === 'debug') return console.log(err.message);
      }
    });
  };
...
module.exports = function(proj, tmpl, cb) {
  ...
  return buildFolders(proj, function() {
    return buildFiles(proj, function() {
      return copyAssets(proj, tmpl, function() {
